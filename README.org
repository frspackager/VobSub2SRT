...
option to vobsub2srt for use with Tesseract 4. Defaults to OEM 1 .

#+BEGIN_EXAMPLE
  vobsub2srt –tesseract-data /home/user/tessdata -–tesseract-oem 1 Vobfile_basename
#+END_EXAMPLE

*** Invert images on the fly

=--invert-image=

Tesseract 4 has been trained for black text on white background, use =--invert-image=
option pass (trivially) inverted images to Tesseract (PGM 255-n per pixel).
